...
if($_SERVER['REQUEST_METHOD'] == "POST" and $tmpl_call_function == "purchase_sms_credit") { 
  ...
  $paid_status = 'A';
  $paid_status_cmnts = "'Direct Approval'";
  ...
  $sql_crdbt = "INSERT INTO user_sms_credit_raise ".
                  "(usrsmscrd_id, user_id, parent_id, pricing_slot_id, expiry_date, valdity_period, raise_sms_credits, 
                  sms_amount, usrsmscrd_comments, usrsmscrd_status, usrsmscrd_status_cmnts, usrsmscrd_entry_date) "."VALUES ".
                  "(NULL, '".$_SESSION['yjwatsp_user_id']."', '".$_SESSION['yjwatsp_parent_id']."', '".$expl[1]."', '$exp_date', 
                  '".$slt_expiry_date."', '".$expl[3]."', '".$hdsms."', '$usrcrdbt_comments', '$paid_status', ".$paid_status_cmnts.", '".$current_date."')";
  if ($conn->query($sql_crdbt)) {
      $cnt_insrt++;
      site_log_generate("Purchase SMS Credit Page : User : ".$_SESSION['yjwatsp_user_name']." Purchase SMS Credit - executed the query ($sql_crdbt) on ".date("Y-m-d H:i:s"), '../');
  }
