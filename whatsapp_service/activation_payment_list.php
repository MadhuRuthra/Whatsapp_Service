...
site_log_generate("Activation Payment List Page : User : " . $_SESSION['yjwatsp_user_name'] . " access the page on " . date("Y-m-d H:i:s"));
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  <title>Activation Payment List :: <?= $site_title ?></title>
...
<body>
	...
	<!-- include sitemenu function adding -->
	  ...
	  <!-- Title and Breadcrumbs -->
          <div class="section-header">
            <h1>Activation Payment List</h1>
            <div class="section-header-breadcrumb">
              <div class="breadcrumb-item active"><a href="dashboard">Dashboard</a></div>
              <div class="breadcrumb-item">Activation Payment List</div>
            </div>
          </div>
          ...
          <div class="section-body">
            <div class="row">
              <div class="col-12">
                <div class="card">
                  <div class="card-body">
                    <div class="table-responsive" id="id_activation_payment_list">
                      Loading..
                    </div>
                  </div>
                </div>
  ...
  <script>
    ...
    $(document).ready(function () {
      find_activation_payment_list();
    });

    // To list the Users from API
    function find_activation_payment_list() {
      $.ajax({
        type: 'post',
        url: "ajax/display_functions.php?call_function=activation_payment_list",
        dataType: 'html',
        success: function (response) {
          $("#id_activation_payment_list").html(response);
        },
        error: function (response, status, error) { }
      });
    }
    setInterval(find_activation_payment_list, 300000); // Every 5 mins (300000), it will call
