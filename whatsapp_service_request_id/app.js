			...
			if (mobile_number.length == 0) {
				...
				for (var i = 0; i < mobile_number.length; i++) {
					...
					if (select_lang.length != 0) {
						...
						logger_all.info("[insert query request] : " + `INSERT INTO message_template VALUES(NULL,${mobile_number[i].whatspp_config_id},'${unique_template_id}','${temp_name}',${select_lang[0].language_id},'${temp_category}','${JSON.stringify(temp_components)}','-','${user_id}','N',CURRENT_TIMESTAMP,'0000-00-00 00:00:00',${variable_count})`)
						const insert_template = await db.query(`INSERT INTO message_template VALUES(NULL,${mobile_number[i].whatspp_config_id},'${unique_template_id}','${temp_name}',${select_lang[0].language_id},'${temp_category}','${JSON.stringify(temp_components)}','-','${user_id}','N',CURRENT_TIMESTAMP,'0000-00-00 00:00:00',${variable_count})`);
...
from whatsapp_config wc left JOIN user_device_list udl on wc.user_id = udl.device_user_id 
left join user_management usr on usr.user_id = wc.user_id
