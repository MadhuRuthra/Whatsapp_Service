    ...
    if (select_query.length > 0) {
      ...
      for (var i = 0; i < array_list_user_id.length; i++) {
        ...
        if (store_id_filter) { //store_id filter using
          get_summary_report = `  SELECT wht.user_id,wht.store_id, usr.user_name,ussr.user_type,wht.whatsapp_entry_date,ml.available_messages, DATE_FORMAT(wht.whatsapp_entry_date,'%d-%m-%Y') entry_date, wht.total_mobileno_count total_msg,wht. total_mobileno_count total_waiting, (select count(distinct comwtap_status_id) from ${newdb}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} where (date(comwtap_entry_date) BETWEEN '${this_date}' AND '${this_date}') and response_status = 'S') total_success, (select count(distinct comwtap_status_id) from ${newdb}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} where (date(comwtap_entry_date) BETWEEN '${this_date}' AND '${this_date}') and response_status = 'S' and delivery_status = 'Y') total_delivered, (select count(distinct comwtap_status_id) from ${newdb}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} where (date(comwtap_entry_date) BETWEEN '${this_date}' AND'${this_date}') and response_status = 'S' and read_status = 'Y') total_read, (select count(distinct comwtap_status_id) from ${newdb}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} where (date(comwtap_entry_date) BETWEEN '${this_date}' AND '${this_date}') and response_status = 'F') total_failed, (select count(distinct comwtap_status_id) from ${newdb}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} where (date(comwtap_entry_date) BETWEEN '${this_date}' AND'${this_date}') and response_status = 'I') total_invalid, (select count(distinct comwtap_status_id) from ${newdb}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} where (date(comwtap_entry_date) BETWEEN '${this_date}' AND '${this_date}') and (response_status not in ('I', 'F', 'S') or response_status is null)) total_waitinggg FROM ${newdb}.compose_whatsapp_tmpl_${array_list_user_id[i]} wht left join ${newdb}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} stt on wht.compose_whatsapp_id = stt.compose_whatsapp_id left join whatsapp_messenger.user_management usr on wht.user_id = usr.user_id left join whatsapp_messenger.message_limit ml on wht.user_id = ml.user_id left join whatsapp_messenger.user_master ussr on usr.user_master_id = ussr.user_master_id where usr.user_id = '${array_list_user_id[i]}' and (date(wht.whatsapp_entry_date) BETWEEN '${this_date}' AND '${this_date}') and wht.store_id = '${store_id_filter}' order by entry_date desc `;
