    ...
    if (select_query.length > 0) {
      ...
      for (var i = 0; i < array_list_user_id.length; i++) {
        ...
        if (store_id_filter) { //store_id filter using
          get_summary_report = `  SELECT wht.user_id,wht.store_id, usr.user_name,ussr.user_type,wht.whatsapp_entry_date,ml.available_messages, DATE_FORMAT(wht.whatsapp_entry_date,'%d-%m-%Y') entry_date, COUNT(CASE WHEN DATE(stt.comwtap_entry_date) BETWEEN '${this_date}' AND '${this_date}' THEN 1 END) AS total_msg, COUNT(CASE WHEN DATE(stt.comwtap_entry_date) BETWEEN '${this_date}' AND '${this_date}' AND stt.response_status = 'S' THEN 1 END) AS total_success,
          COUNT(CASE WHEN DATE(stt.comwtap_entry_date) BETWEEN '${this_date}' AND '${this_date}' AND stt.response_status = 'S' and stt.delivery_status = 'Y' THEN 1 END) AS total_delivered,
            COUNT(CASE WHEN DATE(stt.comwtap_entry_date) BETWEEN '${this_date}' AND '${this_date}' AND stt.response_status = 'S' and stt.read_status = 'Y' THEN 1 END) AS total_read,
             COUNT(CASE WHEN DATE(stt.comwtap_entry_date) BETWEEN '${this_date}' AND '${this_date}' AND stt.response_status = 'F' THEN 1 END) AS total_failed,
              COUNT(CASE WHEN DATE(stt.comwtap_entry_date) BETWEEN '${this_date}' AND '${this_date}' AND stt.response_status = 'I' THEN 1 END) AS total_invalid,
                COUNT(CASE WHEN DATE(stt.comwtap_entry_date) BETWEEN '${this_date}' AND '${this_date}' AND (stt.response_status not in ('I', 'F', 'S') or stt.response_status is null) THEN 1 END) AS total_waiting FROM ${newdb}.compose_whatsapp_tmpl_${array_list_user_id[i]} wht left join ${newdb}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} stt on wht.compose_whatsapp_id = stt.compose_whatsapp_id left join whatsapp_messenger.user_management usr on wht.user_id = usr.user_id left join whatsapp_messenger.message_limit ml on wht.user_id = ml.user_id left join whatsapp_messenger.user_master ussr on usr.user_master_id = ussr.user_master_id where usr.user_id = '${array_list_user_id[i]}' and (date(wht.whatsapp_entry_date) BETWEEN '${this_date}' AND '${this_date}') and wht.store_id = '${store_id_filter}' GROUP BY entry_date order by whatsapp_entry_date desc `;
 ...
 if (getsummary.length == 0) {
  ...
  if (!filter_date && !store_id_filter) {
        ...
        if (filter_date) {
          ...
          function dateRange(startDate, endDate, steps = 1) {
            ...
            while (currentDate <= new Date(endDate)) {
              ...
              get_summary_report += ` SELECT wht.user_id,wht.store_id, usr.user_name,ussr.user_type,wht.whatsapp_entry_date,ml.available_messages,DATE_FORMAT(wht.whatsapp_entry_date,'%d-%m-%Y') entry_date,COUNT(CASE WHEN DATE(stt.comwtap_entry_date) BETWEEN '${slt_date}' AND '${slt_date}' THEN 1 END) AS total_msg, COUNT(CASE WHEN DATE(stt.comwtap_entry_date) BETWEEN '${slt_date}' AND '${slt_date}' AND stt.response_status = 'S' THEN 1 END) AS total_success,
              COUNT(CASE WHEN DATE(stt.comwtap_entry_date) BETWEEN '${slt_date}' AND '${slt_date}' AND stt.response_status = 'S' and stt.delivery_status = 'Y' THEN 1 END) AS total_delivered,
                COUNT(CASE WHEN DATE(stt.comwtap_entry_date) BETWEEN '${slt_date}' AND '${slt_date}' AND stt.response_status = 'S' and stt.read_status = 'Y' THEN 1 END) AS total_read,
                 COUNT(CASE WHEN DATE(stt.comwtap_entry_date) BETWEEN '${slt_date}' AND '${slt_date}' AND stt.response_status = 'F' THEN 1 END) AS total_failed,
                  COUNT(CASE WHEN DATE(stt.comwtap_entry_date) BETWEEN '${slt_date}' AND '${slt_date}' AND stt.response_status = 'I' THEN 1 END) AS total_invalid,
                    COUNT(CASE WHEN DATE(stt.comwtap_entry_date) BETWEEN '${slt_date}' AND '${slt_date}' AND (stt.response_status not in ('I', 'F', 'S') or stt.response_status is null) THEN 1 END) AS total_waiting FROM ${newdb}.compose_whatsapp_tmpl_${array_list_user_id[i]} wht left join ${newdb}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} stt on wht.compose_whatsapp_id = stt.compose_whatsapp_id left join whatsapp_messenger.user_management usr on wht.user_id = usr.user_id left join whatsapp_messenger.message_limit ml on wht.user_id = ml.user_id left join whatsapp_messenger.user_master ussr on usr.user_master_id = ussr.user_master_id where usr.user_id = '${array_list_user_id[i]}' and (date(wht.whatsapp_entry_date) BETWEEN '${slt_date}' AND '${slt_date}') union`;
      ...
      if (!store_id_filter) {
        ...
        logger_all.info('[select query request] : ' + get_summary_report + ` order by whatsapp_entry_date desc `);

        getsummary = await dynamic_db.query(get_summary_report + ` order by whatsapp_entry_date desc `, null, `whatsapp_messenger_${user_id}`);
   }
