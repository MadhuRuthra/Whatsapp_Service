...
// LoginTimeList - start
async function LoginTimeList(req) {
	...
	try {
		...
		var user_id;
		// query parameters
		logger_all.info("[LoginTimeList request query parameters] : " + JSON.stringify(req.body));
		// To get the User_id
		...
		logger_all.info("[select query request] : " + `SELECT log.user_log_status FROM user_log log left join user_management usr on usr.user_id = log.user_id where usr.user_id = '${user_id}' and usr.api_key = '${api_key}' and log.login_time = '${login_time}'`);
		const get_user_log_status = await db.query(`SELECT log.user_log_status FROM user_log log left join user_management usr on usr.user_id = log.user_id where usr.user_id = '${user_id}' and usr.api_key = '${api_key}' and log.login_time = '${login_time}'`);
		//  logger_all.info("[select query response] : " + JSON.stringify(get_user_log_status))
		// if the get message length is '0' to send the no available data.otherwise it will be return the get_user_log_status details.
		if (get_user_log_status.length == 0) {
			return { response_code: 1, response_status: 204, response_msg: 'No data available' };
		}
		else {
			return { response_code: 1, response_status: 200, num_of_rows: get_user_log_status.length, response_msg: 'Success', report: get_user_log_status };
		}

	}
	catch (e) {// any error occurres send error response to client
		logger_all.info("[LoginTimeList failed response] : " + e)
