...
// get_change_password this condition is true.process will be continued. otherwise process are stoped.
			logger_all.info("[select query request] : " + `SELECT * FROM user_management where user_id = '${user_id}' and login_password = '${ex_password}'`);
			...
			logger_all.info("[select query response] : " + JSON.stringify(get_change_password))
// if the get_change_password length is not available to send the Invalid Existing Password. Kindly try again!.otherwise the process was continued
			if (get_change_password.length == 0) {
				...
				logger_all.info("[update query request] : " + `UPDATE user_management SET login_password = '${new_password}' WHERE user_id = '${user_id}'`)
