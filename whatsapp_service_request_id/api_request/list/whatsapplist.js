...
async function WhatsappList(req) {
	...
	try {
		...
		logger_all.info("[whatsapp_list query parameters] : " + JSON.stringify(req.body));
 ...
 // If get_user not available send error response to client in ivalid token
	...
	// get_whatsapp_list to execute the query
			logger_all.info("[select query request] : " + `SELECT wht.compose_whatsapp_id, wht.user_id, usr.user_name, wht.campaign_name, wht.whatsapp_content, wht.message_type, wht.total_mobileno_count, wht.content_char_count, wht.content_message_count, stt.country_code, stt.mobile_no, stt.comments sender, stt.comwtap_entry_date, stt.response_status, stt.response_message, stt.response_id, stt.response_date, stt.delivery_status, stt.read_status FROM whatsapp_messenger_${user_id}.compose_whatsapp_${user_id} wht left join whatsapp_messenger_${user_id}.compose_whatsapp_status_${user_id} stt on wht.compose_whatsapp_id = stt.compose_whatsapp_id left join whatsapp_messenger.user_management usr on wht.user_id = usr.user_id where wht.user_id = '${user_id}' order by wht.compose_whatsapp_id desc, stt.comwtap_status_id desc`);
			const get_whatsapp_list = await db.query(`SELECT wht.compose_whatsapp_id, wht.user_id, usr.user_name, wht.campaign_name, wht.whatsapp_content, wht.message_type, wht.total_mobileno_count, wht.content_char_count, wht.content_message_count, stt.country_code, stt.mobile_no, stt.comments sender, stt.comwtap_entry_date, stt.response_status, stt.response_message, stt.response_id, stt.response_date, stt.delivery_status, stt.read_status FROM whatsapp_messenger_${user_id}.compose_whatsapp_${user_id} wht left join whatsapp_messenger_${user_id}.compose_whatsapp_status_${user_id} stt on wht.compose_whatsapp_id = stt.compose_whatsapp_id left join whatsapp_messenger.user_management usr on wht.user_id = usr.user_id where wht.user_id = '${user_id}' order by wht.compose_whatsapp_id desc, stt.comwtap_status_id desc`);
			logger_all.info("[select query response] : " + JSON.stringify(get_whatsapp_list))
  // if the get message length is '0' to send the no available data.otherwise it will be return the get_messages details.
			if (get_whatsapp_list.length == 0) {
				return {
					response_code: 1,
					response_status: 204,
				...
				return {
					response_code: 1,
					response_status: 200,
					num_of_rows: get_whatsapp_list.length,
					response_msg: 'Success',
					report: get_whatsapp_list
				};
			}
	

	} catch (e) { // any error occurres send error response to client
		logger_all.info("[WhatsappList failed response] : " + e)
		return {
			response_code: 0,
			response_status: 201,
