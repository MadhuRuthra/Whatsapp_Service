...
// ApproveWhatsappNOList - start
async function ApproveWhatsappNOList(req) {
	...
	try {
		...
		var whrcondition = ` `;
		// query parameters
		logger_all.info("[ApproveWhatsappNOList query parameters] : " + JSON.stringify(req.body));
		// To get the User_id
		...
		// get_approve_whatsapp_no to execute this query
		logger_all.info("[select query request] : " + `SELECT wht.whatspp_config_id, wht.user_id, usr.user_name, str.store_id, str.store_owner, str.store_email_address, str.store_address, str.store_mobile_no, wht.mobile_no, wht.qr_code_allowed, wht.whatspp_config_status, city.name city_name, stat.name state_name, cntr.name country_name, wht.country_id, wht.country_code, ctr.phonecode, ctr.shortname FROM whatsapp_config wht left join user_management usr on usr.user_id = wht.user_id left join store_details str on str.store_id = wht.store_id left join master_countries cntr on cntr.id = str.store_country left join master_states stat on stat.id = str.store_state and stat.country_id = cntr.id left join master_cities city on city.id = str.store_city and city.state_id = stat.id left join master_countries ctr on ctr.id = wht.country_id where ${whrcondition} and wht.whatspp_config_status in ('L') and is_qr_code = 'Y' order by wht.whatspp_config_entdate desc`);
		const get_approve_whatsapp_no = await db.query(`SELECT wht.whatspp_config_id, wht.user_id, usr.user_name, str.store_id, str.store_owner, str.store_email_address, str.store_address, str.store_mobile_no, wht.mobile_no, wht.qr_code_allowed, wht.whatspp_config_status, city.name city_name, stat.name state_name, cntr.name country_name, wht.country_id, wht.country_code, ctr.phonecode, ctr.shortname FROM whatsapp_config wht left join user_management usr on usr.user_id = wht.user_id left join store_details str on str.store_id = wht.store_id left join master_countries cntr on cntr.id = str.store_country left join master_states stat on stat.id = str.store_state and stat.country_id = cntr.id left join master_cities city on city.id = str.store_city and city.state_id = stat.id left join master_countries ctr on ctr.id = wht.country_id where ${whrcondition} and wht.whatspp_config_status in ('L') and is_qr_code = 'Y' order by wht.whatspp_config_entdate desc`);
		logger_all.info("[select query response] : " + JSON.stringify(get_approve_whatsapp_no))
		// if the get_approve_whatsapp_no length is not available to send the no available data.otherwise it will be return the get_approve_whatsapp_no details.
		if (get_approve_whatsapp_no.length == 0) {
			return {
				response_code: 1,
				response_status: 204,
			...
			return {
				response_code: 1,
				response_status: 200,
				num_of_rows: get_approve_whatsapp_no.length,
				response_msg: 'Success',
				report: get_approve_whatsapp_no
			};
		}

	} catch (e) { // any error occurres send error response to client
		logger_all.info("[ApproveWhatsappNOList failed response] : " + e)
		return {
			response_code: 0,
			response_status: 201,
...
// ApproveWhatsappNOList - end

// using for module exporting
module.exports = {
	ApproveWhatsappNOList
}