...
async function pricingslot (req) {
	...
	try {
		...
		var user_id;
		// query parameters
		logger_all.info("[get_pricing_slot_status request query parameters] : " + JSON.stringify(req.body));
		// To get the User_id
		...
		logger_all.info("[select query request] : " + `SELECT * FROM pricing_slot where pricing_slot_status = 'Y' `);
		const get_pricing_slot_status = await db.query(`SELECT * FROM pricing_slot where pricing_slot_status ='Y'`);
		 logger_all.info("[select query response] : " + JSON.stringify(get_pricing_slot_status))
		...
		if (get_pricing_slot_status.length == 0) {
			...
			return { response_code: 1, response_status: 200, num_of_rows: get_pricing_slot_status.length, response_msg: 'Success', report: get_pricing_slot_status };
		}
