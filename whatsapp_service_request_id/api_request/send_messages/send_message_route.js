...
This page is used to routing the reply message.
...
// Import the send messages functions page
const replyMsg = require("./reply_msg");
// Import the validation page
const replyMsgValidation = require("../../validation/reply_msg_validation");
...
router.post(
  "/reply_message",
  validator.body(replyMsgValidation),
  valid_user,
  async function (req, res, next) {
    try {// access the replyMsg function
  ...
  if (check_req_id_result.length != 0) {
    ...
    return res.json({ response_code: 0, response_status: 201, response_msg: 'Request already processed', request_id: req.body.request_id });
      ...
      var result = await replyMsg.replyMsg(req);
